...
public final class OperationMessageProcessorBuilder {
  ...
  private TargetOutput targetOutput;
  ...
  public OperationMessageProcessorBuilder setTargetOutput(TargetOutput targetOutput) {
    ...
    return withContextClassLoader(getClassLoader(extensionModel), () -> {
      try {
        ...
        if (operationModel.getModelProperty(PagedOperationModelProperty.class).isPresent()) {
          processor =
              new PagedOperationMessageProcessor(extensionModel, operationModel, configurationProvider, target, targetOutput,
                                                 resolverSet,
                                                 cursorProviderFactory, extensionManager, policyManager,
                                                 extensionConnectionSupplier);
        } else if (supportsOAuth(extensionModel)) {
          processor =
              new OAuthOperationMessageProcessor(extensionModel, operationModel, configurationProvider, target, targetOutput,
                                                 resolverSet,
                                                 cursorProviderFactory, extensionManager, policyManager,
                                                 oauthManager);
        } else {
          processor = new OperationMessageProcessor(extensionModel, operationModel, configurationProvider, target, targetOutput,
                                                    resolverSet,
                                                    cursorProviderFactory, extensionManager, policyManager);
        }
