...
public class ComplexExtensionDeclarationTestCase extends AbstractJavaExtensionDeclarationTestCase {
  ...
  private void assertTargetParameter(ParameterModel target, ParameterModel targetOutput) {
    assertThat(target.getName(), is(TARGET_PARAMETER_NAME));
    assertType(target.getType(), String.class, StringType.class);

    assertThat(targetOutput.getName(), is(TARGET_OUTPUT_PARAMETER_NAME));
    assertType(targetOutput.getType(), TargetOutput.class, StringType.class);
  }
